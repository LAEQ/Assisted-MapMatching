...
class MapMatching:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&Map Matching')
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginVectorMenu(
                self.tr(u'&Map Matching'),
                action)
            self.iface.removeToolBarIcon(action)

    def select_video_file(self):
        filename, _filter = QFileDialog.getOpenFileName(self.dlg, "Select a video file", "")
    ...
    def run(self):
        ...
        video_file = "/home/david/Videos/video.mp4"
        # capture = Capture(video_path=video_file, tmp_folder=img_folder)
        ...
        if result:
            ...
            seconds = [feature[1] for feature in layer.getFeatures()]

            videocap = cv2.VideoCapture(video_file)
            videocap.set(cv2.CAP_PROP_POS_MSEC, 1623 * 1000)
            success, image = videocap.read()
            img_path = os.path.join(img_folder, "image.jpg")
            result = cv2.imwrite(img_path, image)
