...
# @Typo: Matcher
class Matcheur:
    # @comment: arguments must be lowercase: _OID !
    def __init__(self, _network_layer: QgsVectorLayer = None, _path_layer: QgsVectorLayer = None, _OID="OID"):
    ...
    try: 
        ...
        return "error.2"
    except Exception3 as error:
        return "error.3"
    finally:
        any code to be executed exception or success.
    ...
    def find_best_path_in_network(self):
        ...
        """
        @comment: Why a double check:
         In layerTraductor.from_vector_layer_to_list_of_dict line: 27-28 and 
         after each from_vector_layer_to_list_of_dict call. Just check before the method call
         
         @pseudo-code
         if not isinstance(self.network_layer, QgsVectorLayer):
            return "matcheur.find_best_path_in_network.layer_traductor.from_vector_layer_to_list_of_dict.not_a_layer"
        ...
        try:
            matcher = DistanceMatcher(mymap, max_dist_init=dist, max_dist=dist, obs_noise=sigma, obs_noise_ne=sigma * 2,
                                      non_emitting_states=True, only_edges=True)
            states, _ = matcher.match(pts)
        # @comment: PEP8 invalid - Too broad exception
        except:
    ...
    def snap_points_along_line(self, speedField: string, speedlim: float = 1.5,
        ...
        # @bug: if linelayer is a string, or pointslayer ??
        # @comment: why 15 is hard coded - should be an optional parameter
    ...
    def snap_point_to_closest(self):
        ...
        # @bug: if linelayer is a string ?
        # @refactor: 15 is hard coded: It should be an optional arg with 15
        #
        polyline = build_polyline(linelayer, pointslayer, 15,
                                  self.searching_radius, self.sigma)
        if polyline is None:
        ...
        return layerTraductor.from_list_of_dict_to_layer(pointslayer, self.path_layer, "Point",
                                                         "matched point to closest"), too_far_list
    ...
    def snap_point_by_distance(self):
        ...
        point = polyline.interpolate(
            polyline.project(Point(pointslayer[0]["geometry"].x, pointslayer[0]["geometry"].y)))
        ...
        if len(too_far_list) > 0:
            too_far_list.append(len(pointslayer))
        return layerTraductor.from_list_of_dict_to_layer(pointslayer, self.path_layer, "Point",
                                                         "matched point by distance"), too_far_list
