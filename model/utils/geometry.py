...
def reverse_line(line):
    ...
    # @question: difference between from shapely.geometry.LineString and shapely.geometry.linestring.LineString
    """
    from shapely.geometry import LineString
    line = LineString()
    line >>> <shapely.geometry.linestring.LineString object at 0x7fbf7d46fb70>
    """
    if not isinstance(line, shapely.geometry.linestring.LineString) or line == LineString():
...
# @bug: method returns a tuple of point or a tuple Line, None
# @typo get_extremities
...
"""
@refactoring:
    Method should be splitted.
    1. Sub method to prepare/normalize the data
    2. Sub method to run the algo.
    3. this method calls the 2 sub method and handle different scenarios.
"""

# @bug: return None or list -> create 'NoneType' object is not iterable
def cut_line(line, points):
    ...
    if not isinstance(points, list) or points == [] or not isinstance(points[0], shapely.geometry.point.Point):
    ...
    # @comment: what does 0.0001 mean ? It should be a constant with proper naming.
...
def connect_lines(l1, l2):
    ...
    # @comment: not a or not b or c or d ???
    # What is the difference between shapely.geometry.linestring.LineString and LineString
    ...
    # @bug: see comment for get_extremites
